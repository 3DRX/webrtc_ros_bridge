...
class RosVideoCapturer : public rtc::AdaptedVideoTrackSource {
 public:
  RosVideoCapturer(const ImageTransportFactory& it,
...
class RosVideoCapturerImpl
    : public boost::enable_shared_from_this<RosVideoCapturerImpl> {
 public:
  RosVideoCapturerImpl(const ImageTransportFactory& it,
                       const std::string& topic,
                       const std::string& transport);
