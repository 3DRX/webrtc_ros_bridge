...
#include "webrtc_ros/cv_mat_sensor_msgs_image_type_adapter.hpp"
#include "webrtc_ros/visibility_control.h"

#include "./burger.hpp"
#include "./policy_maps.hpp"

RCLCPP_USING_CUSTOM_TYPE_AS_ROS_MESSAGE_TYPE(webrtc_ros::ROSCvMatContainer,
...
class Cam2Image : public rclcpp::Node {
 public:
  ...
  explicit Cam2Image(const rclcpp::NodeOptions& options)
      : Node("cam2image", options), is_flipped_(false), publish_number_(1u) {
    setvbuf(stdout, NULL, _IONBF, BUFSIZ);
    // Do not execute if a --help option was provided
    if (help(options.arguments())) {
      // TODO(jacobperron): Replace with a mechanism for a node to "unload"
      // itself from a container.
      exit(0);
    }
    parse_parameters();
    initialize();
 ...
 private:
  ...
  void initialize() {
    ...
    // transport layer will try ensure that every message gets received in
    // order, or best effort, meaning that the transport makes no guarantees
    // about the order or reliability of delivery.
    qos.reliability(reliability_policy_);
    pub_ = create_publisher<webrtc_ros::ROSCvMatContainer>("a_random_image_topic", qos);
    ...
    auto callback = [this](std_msgs::msg::Bool::ConstSharedPtr msg) -> void {
      ...
      RCLCPP_INFO(this->get_logger(), "Set flip mode to: %s",
                  this->is_flipped_ ? "on" : "off");
    };
  ...
  void timerCallback() {
    ...
    webrtc_ros::ROSCvMatContainer container(frame, header);
  ...
  bool help(const std::vector<std::string>& args) {
    if (std::find(args.begin(), args.end(), "--help") != args.end() ||
      ...
      std::stringstream ss;
      ss << "Usage: cam2image [-h] [--ros-args [-p param:=value] ...]"
         << std::endl;
      ss << "Publish images from a camera stream." << std::endl;
      ss << "Example: ros2 run image_tools cam2image --ros-args -p "
      ...
      ss << "  reliability\tReliability QoS setting. Either 'reliable' "
            "(default) or 'best_effort'";
      ss << std::endl;
      ss << "  history\tHistory QoS setting. Either 'keep_last' (default) or "
            "'keep_all'.";
      ss << std::endl;
      ss << "\t\tIf 'keep_last', then up to N samples are stored where N is "
            "the depth";
      ss << std::endl;
      ss << "  depth\t\tDepth of the publisher queue. Only honored if history "
            "QoS is 'keep_last'.";
      ss << " Default value is 10";
      ss << std::endl;
      ss << "  frequency\tPublish frequency in Hz. Default value is 30";
      ss << std::endl;
      ss << "  burger_mode\tProduce images of burgers rather than connecting "
            "to a camera";
      ss << std::endl;
      ss << "  show_camera\tShow camera stream. Either 'true' or 'false' "
            "(default)";
      ss << std::endl;
      ss << "  device_id\tDevice ID of the camera. 0 (default) selects the "
            "default camera device.";
      ss << std::endl;
      ss << "  width\t\tWidth component of the camera stream resolution. "
            "Default value is 320";
      ss << std::endl;
      ss << "  height\tHeight component of the camera stream resolution. "
            "Default value is 240";
      ss << std::endl;
      ss << "  frame_id\t\tID of the sensor frame. Default value is "
            "'camera_frame'";
      ss << std::endl << std::endl;
      ss << "Note: try running v4l2-ctl --list-formats-ext to obtain a list of "
            "valid values.";
      ss << std::endl;
      std::cout << ss.str();
      return true;
  ...
  void parse_parameters() {
    ...
    reliability_desc.description =
        "Reliability QoS setting for the image publisher";
    ...
    const std::string reliability_param =
        this->declare_parameter("reliability", "reliable", reliability_desc);
    ...
    burger_mode_desc.description =
        "Produce images of burgers rather than connecting to a camera";
    burger_mode_ =
        this->declare_parameter("burger_mode", false, burger_mode_desc);
  ...
  rclcpp::Publisher<webrtc_ros::ROSCvMatContainer>::SharedPtr pub_;
  ...
  /// If true, will cause the incoming camera image message to flip about the
  /// y-axis.
  bool is_flipped_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(webrtc_ros::Cam2Image)
