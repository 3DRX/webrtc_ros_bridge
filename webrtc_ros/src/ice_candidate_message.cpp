...
bool IceCandidateMessage::fromJson(const Json::Value& message_json) {
  if (isIceCandidate(message_json)) {
    if (!WebrtcRosJsonParser::GetStringFromJsonObject(
            message_json, kSdpMidFieldName, &sdp_mid))
      return false;
    if (!WebrtcRosJsonParser::GetIntFromJsonObject(
...
bool IceCandidateMessage::fromIceCandidate(
...
webrtc::IceCandidateInterface* IceCandidateMessage::createIceCandidate() {
  ...
  webrtc::IceCandidateInterface* result =
      webrtc::CreateIceCandidate(sdp_mid, sdp_mline_index, candidate, &err);
  if (!result) {
